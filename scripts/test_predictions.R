...
for (i in 1:915){
  ...
  if (p0==pred){pred_classes_multi[i] <- 'N/A: No relevant content.'}
  else if(p1==pred){pred_classes_multi[i] <- 'Physical Activity'}
  else if(p2==pred){pred_classes_multi[i] <- 'Possible Fatality'}
  else if(p3==pred){pred_classes_multi[i] <- 'Potentially unlawful activity'}
  else {pred_classes_multi[i] <- 'Other claim content'}
}
...
missing_preds <- data.frame(.id = missing_rows,
